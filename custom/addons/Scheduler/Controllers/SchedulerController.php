...
//for coreevnts
use Addons\Scheduler\Support\CalendarEventPresenter;
use FI\Modules\Quotes\Models\Quote;
use FI\Modules\Invoices\Models\Invoice;
use FI\Modules\Payments\Models\Payment;
use Addons\Scheduler\Models\Expense;
use Addons\Scheduler\Models\TimeTrackingProject;
use Addons\Scheduler\Models\TimeTrackingTask;
...
//			})->count();
    ...
    public function calendar()
    {
        ...
        //check for TimeTracking addon
        $addon = Addon::where('name', 'Time Tracking')->first();

        if ($addon && $addon->enabled == 1){
            $coredata = array_merge($coredata,['project' => TimeTrackingProject::statusid('1')]);
            $coredata = array_merge($coredata,['task' => TimeTrackingTask::unbilled()->with(['project', 'timers'])]);
        }
