                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <div class="col-lg-8 col-sm-8">
                                @foreach (\addons\Scheduler\Models\Setting::$coreevents as $entityType => $value)
                                    <div class="checkbox">
                                        <label for="enabledCoreEvents{{ $value}}">
                                            <input name="enabledCoreEvents[]" id="enabledCoreEvents{{ $value}}" type="checkbox" {{ (new \addons\Scheduler\Models\Setting())->isCoreeventEnabled($entityType) ? 'checked="checked"' : '' }} value="{{ $value }}">{{ trans("Scheduler::texts.{$entityType}") }}</label>
                                    </div>
                                @endforeach
                            </div>
                        </div>
